...
import { ItemsProvider } from '../services/items/items';
...
export class ShoppingListPage implements ShoppingListCallback {
  ...
  async ionViewDidEnter() {
    ...
    this.authProvider.getCurrentUser()
      .subscribe(async user => {
        this.loader = await this.loadingProvider.createLoadingDataScreen();
  ...
  private instantiateShoppingListDefaultLocation(currentShoppingList) {
    ...
    // Map to return location
      .pipe(map(locationWithSortedCategoriesByUid => {
  ...
  onListDeleted() {
    ...
    this.shoppingListProvider.deleteShoppingListByUid(idOfListToDelete)
      .then(async () => {
        const confirmMessage = await this.alertProvider.getConfirmAlert(
          'Deleted',
          'List deleted',
          {
            text: 'OK'
   ...
   * Edit provided item
   */
  editItem(shoppingList: ShoppingList, category: ShoppingCategory, item: ShoppingItem, slidingItem) {
    // TODO ALH: Fix!
   ...
   */
  removeItem(shoppingList: ShoppingList, category: ShoppingCategory, item: ShoppingItem, slidingItem) {
   ...
   */
  updateOrderOfItems(indexes, shoppingList: ShoppingList, category: ShoppingCategory) {
