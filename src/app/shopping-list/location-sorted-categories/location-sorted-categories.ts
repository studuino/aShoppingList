...
@Component({
    ...
    styleUrls: ['location-sorted-categories.scss']
})
export class LocationSortedCategoriesPage {
    ...
    constructor(private navParams: ActivatedRoute,
        ...
        this.currentLocationTitle = navParams.snapshot.paramMap.get('locationTitle');
        navParams.data.toPromise().then((data: ShoppingList) => this.currentShoppingList = data);
    }
     ...
     */
    async promptForCategoryRename(category) {
     ...
     */
    updateListOrder(indexes, locationWithSortedCategories) {
        ...
        locationWithSortedCategories.sortedCategories.splice(
            ...
            locationWithSortedCategories.sortedCategories
                .splice(indexes.from, 1)[0]); // Item we are moving (splice returns array of removed items!)
